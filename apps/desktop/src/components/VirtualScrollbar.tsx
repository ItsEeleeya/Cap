...
export default function VirtualScrollbarPrimitive(props: Props) {
    ...
    function installTarget(el: HTMLElement) {
        ...
        cleanupScroll = makeEventListener(el, "scroll", onScroll, { passive: true });
    ...
    function handleThumbDown(e: PointerEvent) {
        ...
        const cleanupMove = makeEventListener(window, "pointermove", (ev: PointerEvent) => {
        ...
        const cleanupCancel = makeEventListener(window, "pointercancel", onPointerEnd);
        cleanupPointerEvents = () => {
            cleanupMove?.();
            cleanupUp?.();
            cleanupCancel?.();
