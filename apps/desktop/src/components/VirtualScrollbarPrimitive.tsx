...
export default function VirtualScrollbarPrimitive(props: Props) {
	...
	function handleTrackClick(e: MouseEvent) {
		...
		const trackLen = orientation() === "horizontal" ? rect.width : rect.height;
	...
	createEffect(() => {
		...
		const observedContent =
			currentTarget.firstElementChild as HTMLElement | null;
	...
	return (
		<div
			...
			class={`pointer-events-none select-none opacity-0 transition-opacity duration-200 data-[visible=true]:opacity-100 ${
				props.class ?? ""
			}`}
			style={props.style}
