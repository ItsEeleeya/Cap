...
function SidebarSplitView(props: ParentProps<{
    persistKey?: string;
    sidebar?: JSX.Element;
    resizable?: boolean;
    defaultSidebarWidth?: number;
    ...
    const [local] = splitProps(mergeProps(props, {
        resizable: true,
    ...
    return (
        <div class={cx("flex h-full min-h-0 w-full overflow-hidden", local.class)}>
            ...
            <main class={cx("min-w-0 min-h-0 overflow-hidden flex-1", local.contentClass)}>
                {local.children}
            </main>
        </div>
    );
