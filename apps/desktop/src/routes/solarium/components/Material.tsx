...
/**
 * A tiny RAF-driven tween, separate from the spring entirely. Used for
 * press-scale because a physical spring and a designed easing curve are
 * different shapes — you can't get a deliberate "ease out" feel by
 * tuning stiffness/damping, you need an actual eased interpolation.
 * Call `to(value, durationMs)` to retarget; `value()` reads the current
 * eased value reactively.
 */
function createTween(
	...
	function to(target: number, durationMs: number): void {
...
export function createElasticSurface(options: ElasticSurfaceOptions = {}) {
	...
	// `transform()` below.
	const pressTween = createTween(1);
	...
	function onPointerDown(e: PointerEvent): void {
		...
		pressTween.to(profile.pressScale, profile.pressInMs);
	}
	...
	function onPointerUp(e: PointerEvent): void {
		...
		setTarget(computeTarget(0, 0));
		pressTween.to(1, profile.pressOutMs);
	}
