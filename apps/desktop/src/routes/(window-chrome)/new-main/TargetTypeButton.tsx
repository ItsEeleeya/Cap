...
function TargetTypeButton(props: TargetTypeButtonProps) {
	...
	return (
		<button
			...
			class={cx(
				"flex flex-1 flex-col items-center justify-end gap-1 rounded-2xl py-2.5 text-center transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-9 focus-visible:ring-offset-2 focus-visible:ring-offset-gray-1",
		...
		>
			<local.Component
				class={cx(
					"size-5 transition-colors apple-vibrancy-fill",
					local.selected ? "text-gray-12" : "text-gray-9",
				)}
			/>
			<p class="text-xs opacity-90">{local.name}</p>
		</button>
	);
}
