...
/// Migration 0 -> 1
/// Rename UUID-named project folders to pretty names using existing helper.
pub async fn migrate(app: &AppHandle) -> Result<(), String> {
    let migrated = migrate_all_uuid_projects(app).await?;
    println!(
        "Schema migration 0→1 complete. Projects migrated: {}",
        migrated
    );
    Ok(())
}
    ...
    let migration_results = futures::stream::iter(uuid_projects)
        .map(|path| async move {
            ...
            match RecordingMeta::load_for_project(&path) {
                Ok(meta) => match migrate_project_filename_if_needed_async(&path, &meta).await {
                    Ok(new_path) => {
                        if new_path != path {
