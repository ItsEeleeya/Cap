...
fn main() {
    ...
    async fn start_server(window: Window) -> Result<u16, String> {
        start(move |url| {
            let _ = window.app_handle().emit("redirect_uri", url);
        })
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            //         event.payload().expect("Error wile openning event payload"),
            //     )
            ...
            //         };
            //         let mut none_item =
            //             CustomMenuItem::new(format!("in_{}_none", id_prefix), "None");
            //         if selected_device.is_none() {
            //             none_item = none_item.selected();
            //         }
            //         let initial = SystemTrayMenu::new().add_item(none_item);
            //         devices.iter().filter(|device| device.kind == kind).fold(
            //             initial,
            //             |tray_items, device| {
            ...
            //     let new_menu = create_tray_menu(Some(vec![
            ...
            //     tray_handle
            //         .set_menu(new_menu)
            //         .expect("Error while updating the tray menu items");
            // });
